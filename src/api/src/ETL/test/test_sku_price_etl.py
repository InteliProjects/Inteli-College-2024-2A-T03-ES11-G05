import pandas as pd
from ETL.sku_price import sku_price_etl  # Ajuste conforme o caminho correto para o fluxo

def test_sku_price_etl():
    # Simulando um DataFrame de SKU Prices
    data = {
        'cod_prod': [123, 456, 789, None, 123],
        'preco': ['100.5', 'abc', '300.0', '200.0', '100.5'],
    ...
    skuPriceDF = pd.DataFrame(data)

    # Executa o fluxo ETL
    cleaned_df = sku_price_etl(skuPriceDF)
    ...
    assert pd.api.types.is_numeric_dtype(cleaned_df['preco']), "A coluna 'preco' não foi convertida corretamente para numérico."
